...
.controller('TrendingCtrl', function($scope, $http, $localStorage){
	...
	//    console.log(day);	
		...
		$http.get(wordpress_url+'/wp-json/wp/v2/posts?filter[orderby]=comment_count&filter[order]=desc&filter[year]='+year+'&filter[monthnum]='+12+'&filter[day]='+day ,{
			params:{"page":$scope.page,"per_page":wordpress_per_page}
		}).then(function(response){
			// console.log('res',response);
			...
			angular.forEach(response.data, function(item){
				if(item.mobiconnector_comments.length !== 0){
...
.controller('DetailCtrl', function($scope, $http, $sce, $stateParams, $ionicPopover, $localStorage, $ionicPopup, IonicClosePopupService){
	...
	$scope.showLoading("Loading...");
	// console.log('data');
	$http.get(wordpress_url+'/api/get_post/?post_id='+$scope.posts)
	.then(function(response){

		$scope.loadDetail = true;
		
		$scope.data = response.data.post;
		// console.log('data',$scope.data);
		$scope.data.time = new Date($scope.data.date).getTime();
		$scope.load = function(){
			$http.get(wordpress_url+'/wp-json/wp/v2/posts?categories='+$scope.data.categories[0].id ,{
	...
	// $http.get(wordpress_url+'/api/get_post/?post_id/'+$scope.posts).then((s)=>{
	// 	console.log('s',s);
	// },(e)=>{
	// 	console.log('e',e);
	// })
...
.controller('CommentCtrl', function($scope, $http, $state, $sce, $stateParams, $localStorage, $ionicScrollDelegate){
	...
	$scope.load = function(){
		$http.get(wordpress_url+'/wp-json/wp/v2/comments?post='+$scope.posts ,{
			...
			angular.forEach(response.data, function(item){
				// $scope.comments.push({item:item});
				//    $scope.fakeArray.push(item);
				console.log('item',item);
				var name = item.author_name;
				var item = item;
				
			// $scope.getFullUserObject(item);				
				
				
	 $http.post(wordpress_url+'/wp-json/mobiconnector/user/get_info/?username='+name,{
		    "username": name
		},{
			...
			// $scope.getFullUserObject(id);
			$http.get(wordpress_url+'/api/user/get_userinfo/?user_id='+id+'&insecure=cool',{
				// params:{"orderBy":"date","order":"desc","page":$scope.page,"per_page":10}
			})
			.then(function(s){
				$scope.avatar = s.data.avatar;
				 
				$scope.comments.push({item:item,avatar:$scope.avatar})

				// console.log('images',$scope.comments);
				// $localStorage.userData = s.data;
				
			},function(e){
			   
			   window.plugins.toast.showShortBottom("Check your internet connection");
		   ...
		   $scope.comments.push({item:item,avatar:$scope.avatar})
            // console.log('err',error.config.data.username +" "+error);
			// window.plugins.toast.showShortBottom(error.config.data.username +" "+ error.data.message);
		});
	});
			$scope.$broadcast('scroll.infiniteScrollComplete');
			$scope.page = $scope.page +1;
	...
	$scope.AddAComment = function(){
		if(angular.isDefined($localStorage.login)){
			...
			// $scope.checkToken().then(function(response){
				var authorName = $localStorage.login.user_nicename;
				 var userPic = $localStorage.userData.avatar;
				 var userUrl= $localStorage.userData.url;

				$http({
					method: 'POST',
					url: wordpress_url+'/wp-json/wp/v2/comments',
					data: {'content':$scope.data.content,'post':$scope.posts,'author_name':authorName,'author_url':'http://www.gamelegant.com/members/'+authorName},
					cache: false,
				...
				// $http.post(wordpress_url+'/wp-json/wp/v2/comments?content='+$scope.data.content+'&post='+$scope.posts+'&author_name='+authorName+"&mobiconnector_local_avatar="+userPic)
				.success(function(s){
					$state.reload();
					// console.log('serer',s)
					$scope.hideLoading();
...
.controller('SettingsCtrl', function($scope, $state, $http, $ionicPopup, $localStorage, IonicClosePopupService, Camera){
	...
	if($scope.login) {
	var Authorization = 'Bearer '+$scope.login.token
	
		$http.post(wordpress_url+'/wp-json/mobiconnector/user/get_info/?username='+$scope.login.user_nicename,{
			...
			$scope.login.avatar = $localStorage.userData.avatar;
			window.plugins.toast.showShortBottom(error.data.message);
		});
		
		$scope.getFullUserObject = function(id){
			$http.get(wordpress_url+'/api/user/get_userinfo/?user_id='+id+'&insecure=cool')
			.then(function(s){
				$scope.login.avatar = s.data.avatar;
				$localStorage.userData = s.data;
...
.controller('LoginCtrl', function($scope, $state, $http, $localStorage, $ionicHistory, base64, $stateParams, ngFB, $cordovaOauth){
	...
	$scope.login = function(){
		...
		$http.post(wordpress_url+'/wp-json/jwt-auth/v1/token', {
			...
			$ionicHistory.goBack();
	        window.plugins.toast.showShortBottom("Authorization Success");

		}).catch(function(error){
			$scope.hideLoading();
			window.plugins.toast.showShortBottom("Invalid credentials");
		});
		// $http.post('http://www.gamelegant.com/wp-json/jwt-auth/v1/token?username='+$scope.data.username+'&password='+$scope.data.password).then(function(response){
		//     $scope.hideLoading();
		// 	$localStorage.login = response.data;
		// 	$localStorage.login.user_nicename = $scope.data.username;
		// 	$scope.base = {};
